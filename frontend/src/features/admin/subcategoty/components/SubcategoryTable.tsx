...
const EmptyState = ({ search }: EmptyStateProps) => {
    ...
    return (
        <div className="py-14 px-10 text-center">
            <div className="mx-auto w-14 h-14 rounded-2xl border border-slate-200 bg-slate-50 flex items-center justify-center shadow-sm
            ...
            <h3 className="mt-5 text-lg font-black tracking-tight text-slate-900 dark:text-white">
                {hasSearch ? "No matching sub-categories" : "No sub-categories yet"}
            </h3>
            ...
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400 max-w-xl mx-auto">
                {hasSearch
                    ...
                    : "Create your first sub-category to organize your products better."}
            </p>
...
const SubCategoryTable = ({ subCategories, isLoading, page, limit, search }: SubCategoryTableProps) => {
    const isEmpty = !isLoading && subCategories.length === 0;
    return (
        <div
            className="
                relative
